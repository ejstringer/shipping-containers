...
### without priority species ------------------------------------------------
## Just copied from above

rna_detected_not_priority <- rna %>% filter(!high_priority) %>%
  select_if(grepl('X20', names(.)) | grepl('best_hit', names(.))) %>% 
  group_by(best_hit) %>% 
  summarise_if(is.numeric, sum) %>% 
  pivot_longer(cols = X20210506_0046:X20210811_2063, names_to = 'sample_id',
  ...
  rename(species = best_hit) %>% 
  group_by(container_id) %>% 
  summarise(total_reads_RNA_nopest = sum(reads_rna),
            richness_RNA_nopest = sum(reads_rna > 0))
head(rna_detected_not_priority)

dna_detected_not_priority <- dna %>% filter(!high_priority) %>%
  select_if(grepl('X20', names(.)) | grepl('best_hit', names(.))) %>% 
  group_by(best_hit) %>% 
  summarise_if(is.numeric, sum) %>%
  pivot_longer(cols = X20210504_0002:X20210813_2107, names_to = 'sample_id',
  ...
  rename(species = best_hit) %>% 
  group_by(container_id) %>% 
  summarise(total_reads_DNA_nopest = sum(reads_dna),
            simpsondna_nopest = simpson(reads_dna),
            shannondna_nopest = shannon(reads_dna),
            richness_DNA_nopest = sum(reads_dna > 0)
            ) %>% 
  left_join(rna_detected_not_priority) 

head(dna_detected_not_priority)
...
### species specific for each species (common_name) -------------------------

specific_detection_spp <- spp %>%  
  filter(sample_method != 'VAC UNDERFLOOR') %>% 
  # filter(!sample_id %in% c('X20210721_1510', 'X20210511_0088')) %>% 
  mutate(across(eDNA_rep_1_Ct:eDNA_rep_3_Ct,
                ~ifelse(is.na(.x),34.56, .x))) %>%  # 3/3 mean Furui
...
nrow(specific_detection)
nrow(specific_detection_spp)/5
## detections --------------------------------------------------------------
pest_detection <- specific_detection %>% 
    left_join(dna_detected) %>% 
  left_join(dna_detected_not_priority)


pest_detection_spp <- specific_detection_spp %>% 
  left_join(dna_detected_not_priority)
...
write.csv(pest_detection_spp, './output/pest_detection_methods_species.csv', row.names = F)
...
## number of countries -------------
  countries <- container_history %>% 
    select(container_id, loading_country) %>% 
    group_by(container_id) %>% 
    summarise(no_countries = n()) 
hist(countries$no_countries)  
## container specs ---------------------------------------------------------
...
container$no_countries %>% summary()
boxplot(container$no_countries)
## save --------------------------------------------------------------------
